/**
 * @file    ignition.cpp  [2-STROKE]
 * @brief   2-stroke ignition: fires every 360°, resonance-shaped advance map
 *
 * Flat-twin firing:  Cyl1 @ 0°,  Cyl2 @ 180°  (within 360° window)
 * Dwell cap: 40°  (4-stroke uses 60°; coil recharge time is halved)
 *
 * Advance map shape
 * ─────────────────
 *  2-strokes are highly sensitive to ignition advance because the time
 *  between ignition and peak pressure is shorter than a 4-stroke.
 *  Advance peaks at the exhaust pipe resonance RPM (~4000) where
 *  scavenging efficiency is highest, then tapers aggressively at both
 *  ends to prevent detonation and ring seizure.
 */
...
/* ── Advance table ──────────────────────────────────────────── */
/* RPM axis extends to 9000 – typical 2-stroke UAV ceiling */
...
/* degrees BTDC – hump at ~4000 RPM (exhaust resonance); tapers at high RPM */
const float IGN_ADVANCE_TABLE[IGN_RPM_BINS][IGN_MAP_BINS] = {
    ...
    /* 4000 */ {28,27,25,23,21,20,18,17,15,14,12,11 }, /* resonance peak */
...
/* 2-stroke flat-twin: both cylinders fire within 360° */
static const uint16_t CYL_TDC_DEG[2] = { 0, 180 };
...
/* angle → µs: 2-stroke one full revolution = 360° per (60/RPM) s */
static uint32_t angle_to_us(float ang, float rpm) {
    if (rpm < 100.0f) return 0;
    return (uint32_t)((ang / 360.0f) * (60.0e6f / rpm));
}

/* Wrap into [0, 360) */
static inline uint16_t wrap360(int32_t a) {
    a %= 360; if (a < 0) a += 360; return (uint16_t)a;
}
...
static void __isr __not_in_flash_func(ign_alarm_isr)(void) {
    ...
    if (act == ALARM_CHARGE) { gpio_put(CYL_GPIO[c], 1); s_cyl[c].coil_charging=true; }
    else if (act == ALARM_FIRE) {
        gpio_put(CYL_GPIO[c], 0);
        s_cyl[c].coil_charging=false; s_cyl[c].armed=false;
    }
}

static void sched_alarm(uint32_t du, AlarmAction_t act, uint cyl) {
    if (du==0 || du>100000) return;
    s_pending_action=act; s_pending_cyl=cyl;
    hardware_alarm_set_target(s_alarm_hw, time_us_64()+du);
}

void ignition_init(void) {
    for (int i=0;i<ENGINE_CYL;i++){
        gpio_init(CYL_GPIO[i]); gpio_set_dir(CYL_GPIO[i],GPIO_OUT);
        gpio_put(CYL_GPIO[i],0); s_cyl[i].coil_charging=false; s_cyl[i].armed=false;
    }
...
void ignition_update(float rpm, float map_rel_kpa, bool synced) {
    ...
    /* Dwell cap: 40° for 2-stroke (vs 60° for 4-stroke) */
    float dwell = IGN_DWELL_MS * rpm * 360.0f / 60000.0f;
    if (dwell > 40.0f) dwell = 40.0f;

    /* 2-stroke: crank angle 0-359° only */
    uint16_t cur = crank_get_angle_tdeg() / 10;
    if (cur >= 360) cur -= 360;

    for (int c=0; c<ENGINE_CYL; c++) {
        int32_t fire   = (int32_t)CYL_TDC_DEG[c] - (int32_t)s_advance_deg;
        int32_t charge = fire - (int32_t)dwell;
        s_cyl[c].fire_deg   = wrap360(fire);
        s_cyl[c].charge_deg = wrap360(charge);
...
void ignition_kill(void) {
    hardware_alarm_cancel(s_alarm_hw); s_pending_action=ALARM_NONE;
    for (int i=0;i<ENGINE_CYL;i++){
        gpio_put(CYL_GPIO[i],0); s_cyl[i].coil_charging=false; s_cyl[i].armed=false;
    }
}
